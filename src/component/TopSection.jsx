import React from "react";
import axios from "axios";

const baseURL = "http://localhost:5000";

function TopSection(){

  const response = async ()=>{
  const fetched = await axios.get(baseURL + "/units");
  // result.push(fetched.data)
  return fetched.data;
  }

  return (
        <div className="header">
        <img height={50} width={275} src="https://hodlinfo.com/static/media/HODLINFO.8f78fc06.png" alt="logo" />
        <div className="btn-container">
          <div className="btn">
            <a type="button" href="#">INR</a>
          </div>
          
            <select className="btn" name="" id="">
              <option value= "">
              <a className="btn" type="button" href="#">{response.length}</a>
              </option>
            </select>
            
          
          <div className="btn">
            <a type="button" href="#">BUY BTC</a>
          </div>
          
        </div>
  
        <div className="right-header">
          <div className="progress-bar">
  
          </div>
  
          <div className="tele-btn">
            <a href="#"> 
            <img style={{marginRight : "5px"}} width={20} height={20} src="data:image/png;base64," alt="" />
              Connect Telegram</a>
          </div>
        </div>
      </div>
    );
}

export default TopSection;